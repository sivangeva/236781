...
class Attack:
    ...
    def gradient_ascent_step(self, pert, data_shape, data_loader, y_list, clean_flow_list,
        ...
        # calculate gradients of the entire data-set
    ...
    # input: as grad_acent original, with the edition pert_diff
    # outpu: same... but we neet the loss !!!
    # inside: different calculation of the pert (from the paprer)
    def gradient_ascent_step_apgd(self, pert, data_shape, data_loader, y_list, clean_flow_list,device=None):
        ...
        for data_idx, data in enumerate(data_loader):
            ...
            motions_gt, scale, pose_quat_gt, patch_pose, mask, perspective = extract_traj_data(data)
            mask1, mask2, perspective1, perspective2 = self.prep_data(mask, perspective)
            # addition: will return loss 
            grad, loss_temp = self.calc_sample_grad(pert_expand, img1_I0, img2_I0, intrinsic_I0,
                                         img1_delta, img2_delta,
                                         scale, y_list[data_idx], clean_flow_list[data_idx], patch_pose,
                                         perspective1, perspective2,
                                         mask1, mask2, device=device)
            ...
            grad = grad.sum(dim=0, keepdims=True).detach()
            # update loss to be summed acrros data points 
            ...
            del perspective
            torch.cuda.empty_cache()
        #print('===> SIVAN, this is loss_total in gradient_ascent_step_apgd')
        #print(loss_total)
        return grad_tot, loss_total
